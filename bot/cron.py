...
def botjob():
    ...
    global x
    p1 = sub.Popen(['minyami', '-d', 'https://fms2.uniqueradio.jp/agqr10/aandg1.m3u8',
                '--live', '--output', f"botfiles/{x}_fresh.ts"])
    try:
...
def backup():
    ...
    global x
    p1 = sub.Popen(['minyami', '-d', 'https://fms2.uniqueradio.jp/agqr10/aandg1.m3u8',
                '--live', '--output', f"botfiles/{x-1}_backup.ts"])
    try:
...
def ffmpegjob():
    ...
    try:
        ...
        sub.run(["ffmpeg", "-y", "-i", "https://fms2.uniqueradio.jp/agqr10/aandg1.m3u8",
                 "-t", "1860", f"botfiles/{x}_backup.mp4"], check=True, timeout=1900)
        x += 1
